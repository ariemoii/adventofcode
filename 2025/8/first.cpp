...
int main() { 
  ...
  int connectionsLeft = NUM_CONNECTIONS;

  for (connection& c : connections) {
    if (!connectionsLeft) break;
    if (!uf.same(c.boxID1, c.boxID2)) {
      if (connectionsLeft == 0) break;
      uf.merge(c.boxID1, c.boxID2);
    }
    connectionsLeft--;
  ...
  for (int i = 0; i < boxes.size(); i++) {
    if (uf.find(i) == i) {
      compSizes.push_back(uf.size[i]);
    }
  }

  sort(compSizes.begin(), compSizes.end(), greater<int>());

  cout << "sizes = " << endl;

  for (auto size : compSizes) {
    cout << size << " ";
  }
  cout << endl;
